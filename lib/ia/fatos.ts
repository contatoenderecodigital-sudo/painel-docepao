...
export function fatosDaCasa(cfg?: { prazoMinimoDias?: number }): FatosDaCasa {
  ...
  // Rendimento so do que tem FONTE na lista unica. A pizza de forma traz
  // `servePessoas` do cardapio impresso; a redonda traz o peso tipico do audio
  // da dona em 19/08/2026. Sem isto, a guarda cortaria a resposta CERTA.
  for (const p of produtosDaCasa()) {
    if (p.servePessoas?.length) serve[p.nome] = p.servePessoas.join(" a ") + " pessoas";
    if (p.pesoTipicoKg?.length) serve[p.nome] = p.pesoTipicoKg.join(" a ") + " kg";
  }
  return {
