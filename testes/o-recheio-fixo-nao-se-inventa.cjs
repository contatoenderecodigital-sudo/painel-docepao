...
fs.writeFileSync(
  ...
  [
    ...
    "    (async () => ({ itens: [{ produto: p.nome, qtd: 50, obs: 'camarao' }] })) as never);",
    "  inventado.push({",
    ...
    "    (async () => ({ itens: [{ produto: p.nome, qtd: 50, obs: p.sabores[0] }] })) as never);",
    "  oDaCasa.push({",
    ...
    "// pior que o defeito que a guarda conserta. Estes tres tem que chegar",
    "// inteiros na comanda.",
    "const legitimos = [];",
    ...
    "    (async () => ({ itens: [{ produto: 'coxinha', qtd: 50, obs }] })) as never);",
