...
fs.writeFileSync(
  ...
  [
    ...
    "    // A MARCA DA PERGUNTA E ESTADO, e nao o texto dela: quem pergunta o peso",
    "    // grava `etapa:peso`, e e por essa marca (a ultima) que o fluxo sabe que",
    "    // a pergunta que acabou de sair foi a do peso (03/09/2026). O texto em",
    "    // `ultimaFala` continua indo pro modelo como contexto.",
    "    etapasJaPerguntadas:['bolo','resto_do_cardapio', ...(/quantos quilos/i.test(String(c.ultimaPergunta ?? '')) ? ['resto_do_cardapio:peso'] : [])],",
