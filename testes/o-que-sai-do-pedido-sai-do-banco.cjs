...
fs.writeFileSync(
  ...
  [
    ...
    "    ['brigadeiro']],",
    "",
    "  ['cancelou UMA de duas pizzas do mesmo nome',",
    "    [it('pizza inteira', 'pizza', 1, 'calabresa'), it('pizza inteira', 'pizza', 1, 'frango com catupiry')],",
    "    [it('pizza inteira', 'pizza', 1, 'frango com catupiry')],",
    "    ['pizza inteira [calabresa]']],",
    "",
    "  ['cancelou UMA de duas coxinhas de recheios diferentes',",
    "    [it('coxinha', 'salgado_frito', 100, 'frango'), it('coxinha', 'salgado_frito', 50, 'calabresa')],",
    "    [it('coxinha', 'salgado_frito', 100, 'frango')],",
    "    ['coxinha [calabresa]']],",
    "",
    "  ['a linha que so ficou MAIS COMPLETA nao saiu',",
    "    [it('pizza inteira', 'pizza', 1, 'calabresa')],",
    "    [it('pizza inteira', 'pizza', 1, 'calabresa | frango com catupiry')],",
    "    []],",
    "",
    "  ['a observacao reescrita em outra ordem nao e linha que saiu',",
    "    [it('trufa', 'docinho', 25, 'forminha branca, morango')],",
    "    [it('trufa', 'docinho', 25, 'morango, forminha branca')],",
    "    []],",
    "];",
    "",
    "const erros = [];",
    "for (const [rotulo, antes, depois, esperado] of CASOS) {",
    "  const marca = (x) => x.produto + (x.obs ? ' [' + x.obs + ']' : '');",
    "  const saiu = itensQueSairam(est(antes), est(depois)).map(marca).sort();",
