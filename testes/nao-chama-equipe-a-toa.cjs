...
fs.writeFileSync(
  ...
  [
    ...
    "  { texto: 'quero falar com a dona' } as never, pensar({ situacao: 'humano' }) as never);",
    "const gente = await responder(festa as never,",
    "  { texto: 'quero falar com a equipe' } as never, pensar({ situacao: 'humano' }) as never);",
    "const naoQuero = await responder(VAZIO as never,",
    "  { texto: 'nao quero falar com a dona, quero 50 coxinha' } as never, pensar({ itens: [{ produto: 'coxinha', qtd: 50 }] }) as never);",
    "",
