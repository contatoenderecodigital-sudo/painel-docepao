...
fs.writeFileSync(
  ...
  [
    "import {",
    "  produtosDaCasa, pedeEscolhaDeSabor, palavrasDaFamilia, paresDeNomeCurtoColidindo,",
    ...
    "}",
    "",
    "const paresExport = paresDeNomeCurtoColidindo();",
    "const falhouEtapa = [];",
    "if (paresExport.length !== pares.length) {",
    "  falhouEtapa.push('paresDeNomeCurtoColidindo()=' + paresExport.length + ' andando a lista=' + pares.length);",
    "}",
    "const falhouFrase = [];",
