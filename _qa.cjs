    ...
    passos: [
      ...
      { diz: "pago no pix", fim: true, checa: [
        { id: "PAGAMENTO-QUANDO-DITO", porque: "a trava não pode derrubar o pagamento que o cliente REALMENTE falou",
          ok: (r, h, extra) => { const p = extra.pedido; if (!p) return "pendente"; return /pix/i.test(p.forma_pagamento || ""); } },
      ] },
    ],
  },
];
...
const REGRAS_GERAIS = [
  ...
  { id: "UMA-PERGUNTA-POR-VEZ", porque: "despejar 3 perguntas faz a pessoa responder só a última", ok: (r) => (r.match(/\?/g) || []).length <= 1 },
